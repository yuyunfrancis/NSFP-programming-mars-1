...
class Stock:
    ...
    def update(self, id: int, change: int):
        ...
        index_of_item = next((index for (index, product) in enumerate(self.products) if product["id"] == id))
        if (self.products[index_of_item]["quantity"] + change >= 0):
            self.products[index_of_item]["quantity"] += change
    ...
    def getProductByID(self, id: int) -> Product:
        ...
        index_of_item = next((index for (index, product) in enumerate(self.products) if product["id"] == id))
        return self.products[index_of_item]
