...
class UserManagement:
    ...
    def get_logged_in_user(self) -> User:
        ...
        for entry in entries:
            ...
            if (logged_in == elements[0]):
                found = True
                return User(elements[0], elements[3], elements[4], bool(elements[5]))
            
        if not found:
            raise(Exception("Invalid data: User logged in status not correctly update"))
